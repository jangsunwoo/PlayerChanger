...
public class GameRepositoy extends Repository<GameData> {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int x = stV.getBlockX(); x <= edV.getBlockX(); x += 16)
                    for (int z = stV.getBlockZ(); z <= edV.getBlockZ(); z += 16) chunkPoz.add(new Pair(x, z));
                MessageUtil.printMsgToAll(ChatMessageType.ACTION_BAR, "청크를 조사 중입니다.");

                final Pair<String,String> blocks = new Pair<String,String>(spawnBlock, assignBlock);
                final Pair<Integer,Integer> rRange = new Pair<Integer, Integer>(stV.getBlockY(), edV.getBlockY());
                ...
                MessageUtil.printConsoleLog(ChatColor.GREEN + "%s : [%d], %s : [%d], %s : [%d]".formatted(
                        "Chest", GameData.chestBlockVectors.stream().count(),
                        spawnBlock, GameData.spawnBlockVectors.stream().count(),
                        assignBlock, GameData.assignBlockVectors.stream().count()
                ));
                this.cancel();
            }
    ...
    private void scanSomeChunks(final ArrayList<ChunkSnapshot> chunks, final Pair<String,String> blocks, final Pair<Integer, Integer> yRange) {
        chunks.parallelStream().forEach(chunk -> getBlocksInChunk(chunk, blocks, yRange));
    }
    ...
    private void getBlocksInChunk(final ChunkSnapshot chunk, final Pair<String,String> materialNames, final Pair<Integer, Integer> yRange) {
        ...
        Set<BlockVector> spawnblocks = new HashSet<>();
        Set<BlockVector> assignblocks = new HashSet<>();
        Set<BlockVector> chestblocks = new HashSet<>();
        ...
        for (int x = minX; x <= maxX; ++x) {
            for (int y = minY; y <= maxY; ++y) {
                for (int z = minZ; z <= maxZ; ++z) {
                    if (materialNames.getFirst().equals(chunk.getBlockType(x&0xF, y, z&0xF))) {
                        spawnblocks.add(new BlockVector(x,y,z));
                    } else if (materialNames.getSecond().equals(chunk.getBlockType(x&0xF, y, z&0xF))) {
                        assignblocks.add(new BlockVector(x,y,z));
                    } else if (Material.CHEST.name().equals(chunk.getBlockType(x&0xF, y, z&0xF))) {
                        chestblocks.add(new BlockVector(x,y,z));
                    }
                }
            }
        }
        GameData.spawnBlockVectors.addAll(spawnblocks);
        GameData.assignBlockVectors.addAll(assignblocks);
        GameData.chestBlockVectors.addAll(chestblocks);
    }
